"""
Evaluate parent_child chunking with Azure AI Search + cost/latency tracking.
"""
import sys
import time
...
def run():
    print("=" * 50)
    print("Parent-Child Chunking + Azure AI Search")
    print("=" * 50)
    ...
    index_chunks(chunks, embeddings, strategy="parent_child")

    print("Retrieving & evaluating...", flush=True)
    t0 = time.time()
    all_retrieved = []
    for q in golden:
        results = vector_search(q["question"], top_k=5, strategy_filter="parent_child")
        class R:
    ...
    result = ExperimentResult(
        strategy="parent_child", config={"parent_size": 1024, "child_size": 256, "child_overlap": 30},
    ...
    delete_all_documents(strategy_filter="parent_child")
