"""
Evaluate recursive chunking with Azure AI Search + cost/latency tracking.
"""
import sys
import time
...
def run():
    print("=" * 50)
    print("Recursive Chunking + Azure AI Search")
    print("=" * 50)
    ...
    index_chunks(chunks, embeddings, strategy="recursive")

    print("Retrieving & evaluating...", flush=True)
    t0 = time.time()
    all_retrieved = []
    for q in golden:
        results = vector_search(q["question"], top_k=5, strategy_filter="recursive")
        class R:
    ...
    result = ExperimentResult(
        strategy="recursive", config={"chunk_size": 512, "overlap": 50},
    ...
    delete_all_documents(strategy_filter="recursive")
