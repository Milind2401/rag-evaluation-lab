"""
Evaluate markdown_aware chunking with Azure AI Search + cost/latency tracking.
"""
import sys
import time
...
def run():
    print("=" * 50)
    print("Markdown-Aware Chunking + Azure AI Search")
    print("=" * 50)
    ...
    index_chunks(chunks, embeddings, strategy="markdown_aware")

    print("Retrieving & evaluating...", flush=True)
    t0 = time.time()
    all_retrieved = []
    for q in golden:
        results = vector_search(q["question"], top_k=5, strategy_filter="markdown_aware")
        class R:
    ...
    result = ExperimentResult(
        strategy="markdown_aware", config={"max_chunk_size": 1024},
    ...
    delete_all_documents(strategy_filter="markdown_aware")
