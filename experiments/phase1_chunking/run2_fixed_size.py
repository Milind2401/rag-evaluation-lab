"""
Run 2: Fixed-size chunking with SEMANTIC relevance evaluation.
Uses embedding similarity instead of substring match.
"""
import sys
import time
...
def run():
    print("=" * 50)
    print("Run 2: Fixed-Size + Semantic Evaluation")
    print("=" * 50)
    ...
    index_chunks(chunks, embeddings, strategy="fixed_size_v2")
    ...
    for q in golden:
        results = vector_search(q["question"], top_k=5, strategy_filter="fixed_size_v2")
    ...
    # Evaluate — substring match (for comparison)
    t0 = time.time()
    ...
    # Evaluate — semantic relevance (new)
    ...
    # Print both
    print("\n--- Substring Match (Run 1 method) ---")
    for k, v in substring_metrics.items():
        print(f"  {k}: {v:.3f}")

    print("\n--- Semantic Relevance (Run 2 method) ---")
    ...
    result = ExperimentResult(
        strategy="fixed_size",
        config={"chunk_size": 512, "overlap": 50, "evaluation": "semantic", "threshold": 0.6},
        chunk_count=stats["chunk_count"],
        avg_chunk_tokens=stats["avg_chunk_tokens"],
        retrieval_metrics={**substring_metrics, **semantic_metrics},
        cost=cost,
        latency=latency,
    )
    result.save(Path(__file__).parent / "results" / "run_2")
    delete_all_documents(strategy_filter="fixed_size_v2")
